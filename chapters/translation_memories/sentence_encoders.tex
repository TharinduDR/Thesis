...
\begin{table}[t] 
	...
	\caption[Examples sentence pairs where sentence encoders performed better than edit distance in the STS task.]{Examples sentence pairs where sentence encoders performed better than edit distance in the STS task. \textbf{GOLD} column shows the score assigned by humans, normalised between 0 and 1. The \textbf{ED} column shows the similarity obtained regarding the edit distance. \textbf{Infersent}, \textbf{USE} and \textbf{SBERT} columns show the similarity obtained by Infersent, Universal Sentence Encoder and SBERT respectively. 
	}
...
With these observations, we answer our \textbf{RQ2}: sentence encoders can improve TM matching and retrieval, especially in the lower fuzzy match scenarios. The proposed process would upgrade the current third-generation TM tools as it provides good results and is very efficient.
